...
conf_align = {
    "ios": run_sequence_aligner.Conf.from_dict(
        dict(**run_sequence_aligner.conf_ios, matching=conf_matching.to_dict())
    ),
    "hl": run_sequence_aligner.Conf.from_dict(
        dict(**run_sequence_aligner.conf_hololens, matching=conf_matching.to_dict())
    ),
}
...
def process_sequence(capture, ref_id, input_path, conf, kind):
    ...
    if len(chunk_ids) == 0:
        if kind == "ios":
            ...
            capture.sessions[sequence_id] = Session.load(
                capture.sessions_path() / sequence_id
            )
    ...
    for session_id in chunk_ids:
        path_trajectory = (
            capture.registration_path() / session_id / ref_id / "trajectory_ba.txt"
        )
    ...
    if num_failed > 0:
        logger.warning(
            "Could not align %d/%d chunks for session %s.",
            num_failed,
            len(chunk_ids),
            sequence_id,
        )
...
def run(
    ...
    if not all(
        (capture.registration_path() / i / "trajectory_refined.txt").exists()
        for i in sequence_ids
    ):
    ...
    run_combine_sequences.run(
        ...
        keyframing=map_keyframing,
        reference_id=ref_id,
    )
    for i, ids in [
        [query_id_phone, query_ids_phone],
        [query_id_hololens, query_ids_hololens],
    ]:
        run_combine_sequences.run(
...
def get_data_Construction():
    ref_id = "navvis_2024-06-28_11.16.52"
    phone_sequences = read_sequence_list(sequence_list_dir / "Construction_phone.txt")
    hololens_sequences = read_sequence_list(
        sequence_list_dir / "Construction_hololens.txt"
    )
    return ref_id, phone_sequences, hololens_sequences


def get_data_CON():
    ref_id = "2024-06-28_11.16.52"
    phone_sequences = read_sequence_list(sequence_list_dir / "CON_phone.txt")
    hololens_sequences = read_sequence_list(sequence_list_dir / "CON_hololens.txt")
    return ref_id, phone_sequences, hololens_sequences
...
def main(args):
    ...
    if args.skip_hololens:
        hololens_sequences = []
    logger.info(
        "Found %d phone and %d HoloLens sequences in lists.",
        len(phone_sequences),
        len(hololens_sequences),
    )
    run(
        args.capture_root / scene,
        ref_id,
        args.phone_dir / scene,
        args.hololens_dir / scene,
        phone_sequences,
        hololens_sequences,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scene", type=str, required=True, choices=["CAB", "HGE", "LIN", "CON"]
    )
    parser.add_argument("--phone_dir", type=Path, default=Path("/media/HD8TB/ios_rec/"))
    parser.add_argument(
        "--hololens_dir", type=Path, default=Path("/media/SSD2/hololens_proc/all-fix")
    )
